...
import type { CommandInteraction, GuildMember, Events } from "discord.js";
import { formatArray, formatPermissions, resolveCommandName } from "@/utils/helpers.js";
...
export default class extends Event {
	...
	public async run(interaction: CommandInteraction<"cached" | "raw">) {
		...
		if (command) {
			if (command.disabled && this.client.developer !== interaction.user.id) {
			...
			if (command.ownerOnly && this.client.developer !== interaction.user.id) {
